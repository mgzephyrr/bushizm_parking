...
def validate_phone(phone: str):
    ...
    if len(phone) != 11 or not phone.startswith("7"):
        raise HTTPException(status_code=422, detail="Phone number must be 11 digits and start with '7'")

async def authenticate_user_by_phone(phone: str, response: Response):
    validate_phone(phone)
    
    if not API_TOKEN:
        raise HTTPException(status_code=500, detail="API token not set in environment")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to get or parse API response: {e}")

    if api_response.status_code != 200:
        raise HTTPException(status_code=api_response.status_code, detail=data)

    users_data = data.get("data", {}).get("userdata", [])
    if not users_data or not isinstance(users_data, list):
        raise HTTPException(status_code=404, detail=f"No valid 'userdata' found in API response: {data}")
